 ...
 */
export function generateGitLabCI(config: PipelineConfig): string {
  return `stages:
  - test
  - build
  - security
...
export const PIPELINE_TEMPLATES: Record<string, { buildTool: string; testCommand: string; lintCommand: string; needsDocker: boolean }> = {
  nodejs: {
    buildTool: 'npm',
