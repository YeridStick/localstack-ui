...
// Usa el servicio híbrido: OpenRouter primero, fallback a Ollama
...
6. MEJORAS_FUTURAS recomendadas`;
  ...
  try {
    const aiResponse = await sendMessageToAI(prompt, model, [
      { role: 'system', content: INFRA_DESIGNER_CONTEXT }
    ]);

    if (!aiResponse.success || !aiResponse.content) {
      throw new Error(aiResponse.error || 'Error en servicio de IA');
    }
...
Responde únicamente con el JSON actualizado, sin explicaciones adicionales.`;
  ...
  try {
    ...
    if (!aiResponse.success || !aiResponse.content) {
      return { success: false, error: aiResponse.error || 'Error en servicio de IA' };
    }
